...
def exporter_vers_csv():
    # Demande de permission à chaque export
    request_permissions([
    ...
    dossier_principal = primary_external_storage_path()
    dossier_export = os.path.join(dossier_principal, "Documents", "BudgetApp")
...
class ConfigurationScreen(Screen):
    ...
    def exporter_vers_excel():
        try:
            # Essayer d'accéder au stockage externe
            try:
                from android.storage import primary_external_storage_path
                dossier_principal = primary_external_storage_path()
            except Exception:
                dossier_principal = "/storage/emulated/0"  # fallback
    
            # Dossier Documents/BudgetApp
            dossier_export = os.path.join(dossier_principal, "Documents", "BudgetApp")
            if not os.path.exists(dossier_export):
                os.makedirs(dossier_export)
            ...
            # Nom du fichier (avec mois et année)
            maintenant = datetime.now()
            fichier_excel = os.path.join(
                dossier_export,
                f"budget_export_{maintenant.strftime('%B_%Y')}.xlsx"
            )
    
            # Sauvegarde
            wb.save(fichier_excel)
            afficher_popup(f"Exportation réussie :\n{fichier_excel}")
    
        except Exception as e:
            erreur = traceback.format_exc()
            afficher_popup(f"Erreur lors de l'export Excel :\n{e}\n\n{erreur}")
