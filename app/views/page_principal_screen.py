...
##########################################################################################
        ...
        self.nom_input = TextInput(hint_text="Nom", multiline=False, size_hint_y=1)
...
##############################              
        ...
        for text, screen_name in buttons_data:
            btn_width = Window.width * 0.2  # 18% de la largeur de l’écran
            btn = Button(
                text=text,
                size_hint=(None, 1),
                width=btn_width
            )
            # Texte centré et taille adaptative
            btn.halign = 'center'
            btn.valign = 'middle'
            btn.text_size = (btn.width, None)
            btn.bind(size=lambda w, s, b=btn: setattr(b, 'font_size', s[1]*0.5))
