...
class pageprincipalScreen(Screen):
    ...
    def mettre_a_jour_fin_de_mois(self):
        total_revenu = 0.0
        total_depense = 0.0
        total_restant_a_payer = 0.0
        ...
        if os.path.exists(self.data_file):
            with open(self.data_file, "r", encoding="utf-8") as f:
                ...
                charges = data.get("charges_a_payer", [])
    
                total_revenu = sum(item.get("montant", 0.0) for item in revenus)
                total_depense = sum(abs(item.get("montant", 0.0)) for item in depenses)
    
                # Construire un set de (nom, date) des dépenses existantes
                depenses_set = set((d.get("nom"), d.get("date")) for d in depenses)
    
                # Calculer total_restant_a_payer en excluant les charges déjà réglées par dépense
                for charge in charges:
                    reste = charge.get("reste_a_payer", 0.0)
                    nom = charge.get("nom")
                    date = charge.get("date")
    
                    if reste < 0:
                        # Si charge à payer toujours en reste
                        if (nom, date) not in depenses_set:
                            total_restant_a_payer += abs(reste)
    
        solde_final = total_revenu - total_depense - total_restant_a_payer
        self.fin_label.text = f"Fin de mois : {solde_final:.2f} €"
    
        if solde_final >= 0:
            self.fin_label.color = (0, 0.6, 0, 1)  # vert
        else:
            self.fin_label.color = (1, 0, 0, 1)    # rouge
    ...
    def ajouter_valeur(self, instance):
        try:
            ...
            # Si c'est une charge fixe, on met à jour les charges à payer
            if categorie == 'charges_fixe':
                ...
                charges_a_payer.append({
                    ...
                    "date": date,
                    "montant": abs(montant),
                    "reste_a_payer": montant  # montant négatif
                })
            ...
            self.mettre_a_jour_labels()
            self.calculer_restant_a_payer()
            self.mettre_a_jour_fin_de_mois()
            self.maj_total_charges_restantes()
    ...
    def mettre_a_jour_restant_a_payer(self):
        # Cette fonction est maintenant simplifiée pour afficher/masquer le label
        app = App.get_running_app()
        if hasattr(self, 'total_charges_restantes_label'):
            if app.show_restant_a_payer:
                self.total_charges_restantes_label.opacity = 1
                self.total_charges_restantes_label.height = 50
            else:
                self.total_charges_restantes_label.opacity = 0
                self.total_charges_restantes_label.height = 0
    ...
    def mettre_a_jour_labels(self):
        ...
        # Calcul du solde
        solde = total_revenus + total_depenses  # attention : total_depenses est négatif dans les données
    
        self.solde_label.text = f"Solde actuel : {solde:.2f} €"
    
        # ✅ Changement de couleur selon le solde
        if solde >= 0:
            self.solde_label.color = (0.2, 0.4, 1, 1)  # bleu doux
        else:
            self.solde_label.color = (1, 0, 0, 1)    # rouge
