...
class EpargneScreen(Screen):
    def __init__(self, **kwargs):
        ...
        scrollview = ScrollView(size_hint=(1, None), height=240)
        contenu_formulaire = GridLayout(cols=1, size_hint_y=None, spacing=10, padding=10)
        contenu_formulaire.bind(minimum_height=contenu_formulaire.setter('height'))

        input_wrapper = BoxLayout(size_hint=(1, None), height=220, padding=2)
        ...
        self.nom_input = self.creer_champ(inner_layout, "Nom", 80)
        self.montant_input = self.creer_champ(inner_layout, "Montant", 80)
        ...
        scrollview2 = ScrollView(size_hint=(1, None), height=150)
        contenu2 = GridLayout(cols=1, size_hint_y=None, spacing=1)
        contenu2.bind(minimum_height=contenu2.setter('height'))

        input_wrapper2 = BoxLayout(size_hint=(1, None), height=105, padding=2)
        with input_wrapper2.canvas.before:
            Color(0, 0, 0, 1)
            self.border_rect2 = Rectangle()

        input_wrapper2.bind(size=lambda w, s: setattr(self.border_rect2, 'size', s),
                            pos=lambda w, p: setattr(self.border_rect2, 'pos', p))

        inner_layout2 = BoxLayout(orientation='vertical', spacing=5, padding=10)
        with inner_layout2.canvas.before:
            Color(0.95, 0.95, 0.95, 1)
            self.bg_rect2 = Rectangle()

        inner_layout2.bind(size=lambda w, s: setattr(self.bg_rect2, 'size', s),
                           pos=lambda w, p: setattr(self.bg_rect2, 'pos', p))

        self.livret_input = self.creer_champ(inner_layout2, "Montant livret", 80)
    ...
    color=(0, 0, 0, 1)
        ...
        self.btn_toggle = Button(text='Afficher les Épargnes', size_hint_y=None, height=50)
        self.btn_toggle.bind(on_release=self.toggle_epargnes)
        self.layout.add_widget(self.btn_toggle)

        self.table_container = ScrollView(size_hint=(1, None), height=0)
    ...
    def charger_epargnes(self):
        ...
        self.table_layout.add_widget(Label(text="[b]Modifier[/b]", markup=True, color=(0, 0, 0, 1)))
        self.table_layout.add_widget(Label(text="[b]Supprimer[/b]", markup=True, color=(0, 0, 0, 1)))
        ...
        for index, item in enumerate(epargnes):
            ...
            btn_modifier = Button(text="Modifier", background_color=(0.8, 0.8, 0.2, 1))
            ...
            btn_supprimer = Button(text="Supprimer", background_color=(1, 0.4, 0.4, 1))
    ...
    def afficher_confirmation_suppression(self, index):
        ...
        message = Label(
            text="Confirmer la suppression de cette epargne ?",
            size_hint=(1, None),
            height=80,
            halign="center",
            valign="middle"
