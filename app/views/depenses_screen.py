...
class DepenseScreen(Screen):
    ...
    def on_pre_enter(self):
        ...
        for index, item in enumerate(depenses):
            ...
            btn_suppr.bind(on_press=partial(self.supprimer_depense, index))
    ...
    def supprimer_depense(self, index, *args):
        ...
        if 0 <= index < len(depenses):
            ...
            if item_to_remove in soldes:
                ...
                # Si c'était une dépense venant d'une charge, on la remet dans 'a_payer'
                try:
                ...
                if 'a_payer' not in donnees:
                    donnees['a_payer'] = []
    
                deja_existe = any(
                    ap['nom'] == item_to_remove['nom'] and ap['montant'] == item_to_remove['montant']
                    for ap in donnees['a_payer']
                )
                if not deja_existe:
                    donnees['a_payer'].append({
                        'nom': item_to_remove['nom'],
                        'date': item_to_remove['date'],
                        'montant': item_to_remove['montant'],
                        'type': 'a_payer'
                    })
