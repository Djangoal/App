...
class pageprincipalScreen(Screen):
    ...
    def ajouter_valeur(self, instance):
        ...
        try:
            ...
            nouvel_item = {"nom": nom, "date": date, "montant": montant, "type": type_valeur}
            ...
            if os.path.exists(chemin):
                with open(chemin, "r", encoding="utf-8") as f:
                    try:
                        ...
                        donnees = {"soldes": [], "a_payer": []}
            else:
                donnees = {"soldes": [], "a_payer": []}
                
    
            # Ajouter la nouvelle entrée dans 'soldes'
            donnees.setdefault("soldes", []).append(nouvel_item)
    
            # Sauvegarder dans le fichier
            with open(chemin, "w", encoding="utf-8") as f:
                json.dump(donnees, f, ensure_ascii=False, indent=4)
    
            # Mettre à jour l'affichage en mémoire
            self.soldes = donnees["soldes"]
            self.mettre_a_jour_labels()
        ...
        if type_valeur in ["charges_fixe", "depense"]:
            try:
                with open("donnees_budget.json", "r", encoding="utf-8") as f:
                    donnees = json.load(f)
        
                liste_a_payer = donnees.get("a_payer", [])
        
                # Chercher une ligne correspondante (nom + montant)
                for item in liste_a_payer:
                    if item["nom"] == nom and abs(item["montant"]) == abs(montant):
                        liste_a_payer.remove(item)
                        break
                ...
                with open("donnees_budget.json", "w", encoding="utf-8") as f:
                    ...
                    # Mise à jour des données en mémoire et des labels
                self.a_payer = donnees.get("a_payer", [])
                self.mettre_a_jour_labels()
                
        
            except Exception as e:
                print("Erreur suppression a_payer :", e)
    ...
    def mettre_a_jour_labels(self):
        ...
        # Calcul du total "a_payer"
        a_payer = 0
        try:
            with open('donnees_budget.json', 'r', encoding='utf-8') as f:
                donnees = json.load(f)
    
            for item in donnees.get("a_payer", []):  # 🔧 <- correction ici
                try:
                    a_payer += float(item.get('montant', 0))
                except (ValueError, TypeError):
                    continue
        except Exception as e:
            print("Erreur lors de la lecture de donnees_budget.json :", e)
            
        # Mettre à jour les labels
        self.label_restant_a_payer.text = f"Restant à payer : {abs(a_payer):.2f} €"
        ...
        solde = total_revenus + a_payer + total_depenses
