...
from datetime import datetime  # N’oublie pas d’importer datetime en haut du fichier
...
class pageprincipalScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.depense_cbox = CercledCheckbox("Dépense")
        ...
        # Bouton simulation de crash
        self.crash_btn = Button(
            text="Simuler Crash",
            size_hint=(1, None),
            height=100,
            background_color=(1, 0, 0, 1)
        )
        self.crash_btn.bind(on_press=self.simuler_crash)
        main_layout.add_widget(self.crash_btn)
        ...
        self.update_affichage_revenus(app, getattr(app, 'show_total_revenus', True))
        self.update_affichage_charges(app, getattr(app, 'show_total_charges', True))
        self.update_affichage_depenses(app, getattr(app, 'show_total_depenses', True))

    def simuler_crash(self, *args):
        with open("/storage/emulated/0/mon_app/log_crash.txt", "a") as f:
            f.write("Simulation de crash déclenchée à " + str(datetime.now()) + "\n")
        raise RuntimeError("Simulation de crash déclenchée !")
    ...
    def mettre_a_jour_labels(self):
        ...
        self.label_charges.text = f"Total Charges Fixes : {total_charges:.2f} €"
        self.label_depenses.text = f"Total Dépenses : {total_depenses:.2f} €"
