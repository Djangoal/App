...
class DepenseScreen(Screen):
    ...
    def on_pre_enter(self):
        print("[DepenseScreen] => Entrée dans l'écran")
        ...
        print(f"[DepenseScreen] => {len(depenses)} dépense(s) trouvée(s)")

        for index, item in enumerate(depenses):
            print(f"[DepenseScreen] => Ajout de la dépense : {item}")
        ...
        print(f"[DepenseScreen] => Total recalculé : {total_depenses:.2f} €")
        self.total_depenses_label.text = f"Total des dépenses : {total_depenses:.2f} €"

    def supprimer_depense(self, index):
        print(f"[DepenseScreen] => Demande de suppression de l'index : {index}")
        app = App.get_running_app()
        principal_screen = app.root.get_screen('principal')
