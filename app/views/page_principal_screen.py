...
##########################################################################################
    ...
    ##########################################################################################    
        ...
        # --- Création des cases avec texte dedans
        self.revenu_testbox = TextCheckbox(text="Revenu", group="categorie",size_hint=(0.3, 0.3),allow_no_selection=True)
        self.charges_fixe_testbox = TextCheckbox(text="Charge", group="categorie",size_hint=(0.3, 0.3),allow_no_selection=True)
        self.depense_testbox = TextCheckbox(text="Dépense", group="categorie",size_hint=(0.3, 0.3),allow_no_selection=True)
        ...
        # --- Ajout du conteneur dans le cadre
...
# ...
    ...
    def ajouter_valeur(self, instance):
        try:
            ...
            if self.revenu_testbox.state == 'down':
                categorie = 'revenu'
                # montant reste positif
            elif self.charges_fixe_testbox.state == 'down':
                categorie = 'charges_fixe'
                montant = -abs(montant)
            elif self.depense_testbox.state == 'down':
                categorie = 'depense'
                montant = -abs(montant)
            else:
                return
            ...
            self.revenu_testbox.state = 'normal'  # décoché
            self.charges_fixe_testbox.state = 'normal'
            self.depense_testbox.state = 'normal'
