...
class pageprincipalScreen(Screen):
    ...
    def mettre_a_jour_labels(self):
        ...
        restant_a_payer = self.lire_total_charges_a_payer()
        #restant_a_payer = total_charges + total_depenses
        
        with open("donnees_budget.json", "r") as f:
             data = json.load(f)
             data["total_charges_a_payer"] = restant_a_payer
        with open("donnees_budget.json", "w") as f:
             json.dump(data, f, indent=4)
        
        
        #self.label_restant_a_payer.text = f"Restant à payer : {restant_a_payer:.2f} €"
        self.label_restant_a_payer.text = f"Restant à payer : {abs(restant_a_payer):.2f} €"
        ...
        solde = total_revenus + restant_a_payer + total_depenses
