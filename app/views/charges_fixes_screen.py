...
class ChargesFixesScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # === Section bouton pour afficher/masquer les charges fixes ===
        ...
        # Conteneur pour le tableau des charges fixes (masqué par défaut)
        ...
        # ScrollView inutile ici (aucun contenu ajouté dans le GridLayout) – peut être supprimé
        ...
        # === Section bouton pour afficher/masquer les charges à payer ===
        ...
        # Conteneur pour le tableau des charges à payer (masqué par défaut)
        ...
        # ScrollView inutile ici également
        scrollview2 = ScrollView()
        contenu = GridLayout(cols=1, size_hint_y=None, spacing=10)
        contenu.bind(minimum_height=contenu.setter('height'))
        scrollview2.add_widget(contenu)

        # Affichage du total restant à payer
        ...
        # États d’affichage pour les sections masquables
        self.fixes_visible = False
        self.payer_visible = False

    # === Affiche ou masque la section des charges fixes ===
    def toggle_charges_fixes(self, instance):
        max_height = 900
        if self.table_fixes_container.height == 0:
            self.table_fixes_container.height = min(self.table_layout.height, max_height)
    ...
    # === Affiche ou masque la section des charges à payer ===
    def toggle_charges_a_payer(self, *args):
        self.payer_visible = not self.payer_visible
        max_height = 900
        self.table_payer_container.height = min(self.payer_layout.height, max_height) if self.payer_visible else 0
    ...
    def ouvrir_popup_modification(self, index):
        ...
        date_input = TextInput(text=charge['date'], multiline=False, hint_text="Date")
        nom_input = TextInput(text=charge['nom'], multiline=False, hint_text="Nom")
        montant_input = TextInput(text=str(charge['montant']), multiline=False, hint_text="Montant")
